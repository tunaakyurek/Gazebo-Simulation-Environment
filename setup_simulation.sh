...
start_full_simulation() {
    echo "🚀 Starting full simulation environment..."
    ...
    if ps -p $GZ_PID > /dev/null && ps -p $BRIDGE_PID > /dev/null && ps -p $PX4_PID > /dev/null && ps -p $MAVROS_PID > /dev/null; then
        echo "✅ Full simulation environment running successfully"
        echo "📡 ROS topics available:"
        unset ROS_DISTRO
        bash -c "source /opt/ros/jazzy/setup.bash && ros2 topic list" | head -10
        echo
        echo "🎮 Full simulation ready for EKF testing!"
        echo "   - Gazebo PID: $GZ_PID"
        echo "   - Bridge PID: $BRIDGE_PID"
        ...
        # Keep simulation running
        trap "echo 'Stopping simulation...'; kill $GZ_PID $BRIDGE_PID $PX4_PID $MAVROS_PID 2>/dev/null; exit 0" INT
        wait
    else
        echo "❌ Some simulation components failed to start"
        cleanup
